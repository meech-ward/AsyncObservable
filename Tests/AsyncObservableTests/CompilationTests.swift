...
struct AsyncObservableCompilationTests {
  ...
  func testStandardTypes() async {
    // Test with String
    let stringObservable = await AsyncObservable("test")
    #expect(stringObservable.value == "test")
    
    // Test with Bool
    let boolObservable = await AsyncObservable(true)
    #expect(boolObservable.value)
    
    // Test with Double
    let doubleObservable = await AsyncObservable(3.14)
    #expect(doubleObservable.value == 3.14)
    #expect(abs(doubleObservable.value - 3.14) < 0.00001)
    
    // Test with Date
    let now = Date()
    let dateObservable = await AsyncObservable(now)
    #expect(dateObservable.value == now)
    
    // Test with UUID
    let uuid = UUID()
    let uuidObservable = await AsyncObservable(uuid)
    #expect(uuidObservable.value == uuid)
    
    // Test with URL
    let url = URL(string: "https://example.com")!
    let urlObservable = await AsyncObservable(url)
  ...
  func testCustomTypes() async {
    ...
    let pointObservable = await AsyncObservable(Point(x: 1.0, y: 2.0))
    ...
    let personObservable = await AsyncObservable(Person(name: "Alice"))
  ...
  func testCollectionTypes() async {
    // Test with Array
    let arrayObservable = await AsyncObservable([1, 2, 3])
    #expect(arrayObservable.value.count == 3)
    
    // Test with Dictionary
    let dictObservable = await AsyncObservable(["a": 1, "b": 2])
    #expect(dictObservable.value.count == 2)
    
    // Test with Set
    let setObservable = await AsyncObservable(Set([1, 2, 3]))
    #expect(setObservable.value.count == 3)
    
    // Test with nested collections
    let nestedObservable = await AsyncObservable([[1, 2], [3, 4]])
  ...
  func testOptionalTypes() async {
    ...
    let optionalObservable = await AsyncObservable<Int?>(42)
    ...
    let stringOptionalObservable = await AsyncObservable<String?>(nil)
